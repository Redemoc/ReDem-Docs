...
<Update label="2026-09-01">
  ## Credit Calculation
  - <code style={{ color: "green" }}> POST /creditCalculation </code> (v1, v2, v3) — Prefer `GQSDataPoints`. `GQSAnswers` still works as a legacy item count: the API sends `floor(GQSAnswers / 5)` to FinOps as `GQSDataPoints`. Sending both fields returns 400.
