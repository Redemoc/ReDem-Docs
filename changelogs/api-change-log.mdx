...
<Update label="2026-09-24">
  ...
  - `surveyDescription` on a CHS datapoint can include custom exclusion rules as well as survey context.
  - Question text that appears **3 or more times** in the interview is omitted from the coherence evaluation.

  ## Open-Ended Score

  - An open-ended answer that contains an em dash (`—`) is returned as `AI_SUSPECT`. A hyphen (`-`) does not trigger this.

  ## Duplicate Entrance Score

  - A peer comparison scores **100** when either respondent has a demographic `type` the other lacks, even if every shared demographic matches.

  See the [Coherence Score](/knowledge-base/quality-checks/coherence-score), [Open-Ended Score](/knowledge-base/quality-checks/open-ended-score), and [Duplicate Entrance Score](/knowledge-base/quality-checks/duplicate-entrance-score) guides.
</Update>

<Update label="2026-09-17">
  ## Duplicate Entrance Score

  - <code style={{ color: "green" }}> POST /v3/addRespondent </code> — DES can run as an **IP-only** check.
  - `INSUFFICIENT_DEMOGRAPHICS` now means there was **no IP and fewer than 4 demographics**. An IP with fewer than 4 demographics is a valid IP-only check.
  - After a Quick Import, the first respondent in a `DUPLICATE_ENTRANT` chain is also marked as a duplicate (previously left as `VALID_ENTRANT`).

  See the [Duplicate Entrance Score](/knowledge-base/quality-checks/duplicate-entrance-score) guide.
</Update>

<Update label="2026-09-03">
  ## Missing score reason

  When a quality-check datapoint score is `-999` (the check did not produce a real 0–100 score), the API now includes `missingScoreReason`. Existing `status`, `error`, `reason`, and categories are unchanged.

  - Present on `dataPointsSummary` for TS, OES, BAS, GQS, and DES.
  - For **CHS**, the code is on `qualityScoreSummary` (CHS has no datapoint summary).
  - Example codes: `TIMEOUT`, `RATE_LIMIT`, `CONTENT_FILTER`, `MAX_RETRIES`, `UNAVAILABLE`, `INSUFFICIENT_RESPONDENTS`, `MISSING_DURATION`, `INSUFFICIENT_INTERACTION`, `INSUFFICIENT_DEMOGRAPHICS`.

  See [DataPointsQualityResponse](/api-reference/openapi) and the quality-check guides.
</Update>

<Update label="2026-09-01">
  ## Grid-Question Score
  - GQS credits are calculated per **data point**, not per `gridAnswersPattern` item. Modular: **0.2 credits** per GQS data point, capped at **2**. All-inclusive includes **5** GQS data points.
  - `gridAnswersPattern` no longer has a maximum length of 50. The minimum of 5 items remains.
  - All-inclusive quality-check suggestions now cap GQS at **5** (was 20). The API maximum is still 20 GQS data points.
  - <code style={{ color: "green" }}> POST /creditCalculation </code> (v1, v2, v3) — Prefer `GQSDataPoints`. `GQSAnswers` is a legacy alias. Sending both fields returns 400.
  - Response `calculationBreakdown.GQS` uses `dataPoints` when you send `GQSDataPoints`, and echoes `answers` when you send `GQSAnswers`.
</Update>

<Update label="2026-08-27" description="v3.1.10">
  ## Quality Check Suggestions

  - <code style={{ color: "green" }}> POST /v3/qualityCheckSuggestions </code> — Now returns **DES** demographic mappings (`type` + `questionId`) when suitable demographics are present.
...
<Update label="2026-08-07" description="v3.1.9">
  ## Duplicate Entrance Score

  - <code style={{ color: "green" }}> POST /v3/addRespondent </code> — Send a `DES` datapoint (v3 only) with optional `entranceTime`, optional `ip`, and up to 10 demographics (at least 4 for a valid score). At most one DES datapoint per respondent.
  - DES detects likely duplicate entrants by comparing demographics and IP against recent peers in the same survey.
  - <code style={{ color: "green" }}> POST /creditCalculation </code> — Optional `DESDataPoints` (default `0`) for estimating Duplicate Entrance Score credits. Existing clients that omit the field keep working.

  See the [v3 addRespondent](/api-reference/endpoints/v3/addRespondent) endpoint documentation, [creditCalculation](/api-reference/endpoints/v3/creditCalculation), and the [Duplicate Entrance Score](/knowledge-base/quality-checks/duplicate-entrance-score) guide.

  ## Quality Check Suggestions

  - <code style={{ color: "green" }}> POST /v3/qualityCheckSuggestions </code> — New endpoint for partners: send a survey questionnaire structure and receive smart quality-check suggestions (OES, GQS, CHS, BAS) plus recommended cleaning settings.
  ...
  - <code style={{ color: "green" }}> POST /v3/addRespondent </code> — On an OES datapoint, set `skipTranslationLanguages` to a list of ISO 639-1 language codes that should **not** be translated when `shouldTranslate` is `true`.
...
<Update label="2026-07-31" description="v3.1.8">
  ## OES Answer Translation

  - <code style={{ color: "green" }}> POST /v3/addRespondent </code> — Set `shouldTranslate: true` on an OES datapoint to translate non-valid, non-English answers to English during quality evaluation. This makes it easy to review and verify our results for any language.
  ...
  - <code style={{ color: "green" }}> POST /v1/addRespondent </code>, <code style={{ color: "green" }}> POST /v2/addRespondent </code>, <code style={{ color: "green" }}> POST /v3/addRespondent </code>
...
<Update label="2026-07-09" description="v3.1.7">
  ## BAS: Mouse Movement and Click Tracking

  - <code style={{ color: "green" }}> POST /v3/addRespondent </code> — BAS `interactionData` now accepts two additional interaction types alongside keystrokes and copy-paste:
...
<Update label="2026-05-22" description="v3.1.6">
  ## New Attribute: retentionDays

  - <code style={{ color: "green" }}> POST /v3/addRespondent </code> - You can now send an optional `retentionDays` in the request body. It is the number of days to retain the respondent's data. After the retention period is over the respondent will be deleted from ReDem. The data will be no longer available in our platform. The minimum value is 30 days.
