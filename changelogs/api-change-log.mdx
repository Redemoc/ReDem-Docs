...
<Update label="2026-08-27">
  ...
  When a quality-check datapoint score is `-999` (the check did not produce a real 0–100 score), the API now includes `missingScoreReason` — an uppercase machine code. Valid scores omit the field. Existing `status`, `error`, `reason`, and categories are unchanged.
  ...
  - Clients should map codes to display text. ReDem UI does this mapping.
