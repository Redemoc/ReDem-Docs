...
<Update label="2026-08-27" description="v3.1.11">
  ## Ignore Empty Respondent Attributes
  ...
  - `ignoreEmptyRespondentAttributes` — When `true`, empty `respondentAttributes` values (including whitespace-only and `null`) are dropped instead of rejecting the request. Valid attributes are kept. If every attribute is empty, `respondentAttributes` is removed. Defaults to `false`.
