...
<Update label="2026-08-13" description="v1.6.9">
  ## File Import
  - Quick Import quality-check suggestions are now much more accurate and finish faster.

  ## Duplicate Entrance Score
  - You can now configure the **Duplicate Entrance Score (DES)** in Quick Import. Map demographic columns (and optionally IP) the same way you configure other quality checks.
  - DES is **not** suggested automatically by the AI assistant — add it yourself during import.
  ...
  ## BAS
  - The Behavioral Analytics Score now flags more unnatural keystroke patterns. A single keystroke that inserts many words at once (10 or more) is classified as `UNNATURAL_TYPING` with a score of **0**.
