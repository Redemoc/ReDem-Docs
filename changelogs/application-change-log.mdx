...
<Update label="2026-09-24">
  ## Coherence Score
  - In Quick Import, use **Survey Description & Exclusion Rules** for survey context and for custom rules, such as removing respondents who pick a specific answer. ReDem keeps that text with the import settings.
  - Questions whose text appears **3 or more times** are left out of the coherence check. Repeated wording with different unseen stimuli, such as several ads, is no longer treated as a contradiction.
  - See the [Coherence Score guide](/knowledge-base/quality-checks/coherence-score).

  ## Open-Ended Score
  - An answer that contains an **em dash** (`—`) is classified as `AI_SUSPECT`, including short answers. A hyphen (`-`) does not trigger this.
  - See the [Open-Ended Score docs](/knowledge-base/quality-checks/open-ended-score#5-ai-suspect-probable-non-human-generated-response).

  ## Duplicate Entrance Score
  - If one respondent has a demographic type the other does not, that pair scores **100** (not a duplicate), even when the shared demographics match.
  - See the [Duplicate Entrance Score guide](/knowledge-base/quality-checks/duplicate-entrance-score).
</Update>

<Update label="2026-09-17" description="v1.7.4">
  ## Duplicate Entrance Score
  - You can now run DES as an **IP-only** check.
  - After a Quick Import, the **first respondent** in a `DUPLICATE_ENTRANT` chain is also marked as a duplicate. Previously that original entrant stayed `VALID_ENTRANT` even though later respondents pointed at them.
  - See the [Duplicate Entrance Score guide](/knowledge-base/quality-checks/duplicate-entrance-score) and [Quick Import](/features/quick-import) for details.

  ## Quick Import
  - When you select columns, the picker now always shows both the **Header ID** and the **Question Text**. You can search either value.

  ## BAS
  - Fixed a bug in mouse movement detection that could flag genuine behaviour as `UNNATURAL_MOVEMENT` on **4K+** displays and with very fast mouse movements.
</Update>

<Update label="2026-09-07" description="v1.7.3">
  ## Grid-Question Score billing
  - GQS is now billed per **data point** (grid), not per grid item. Modular: **0.2 credits** per grid, capped at **2**. All-inclusive includes **5** grids; extra grids are 0.2 each, still capped at 2.
  - Import no longer blocks grids with more than 50 items. Grids still need at least 5 items (7 for pattern check).
</Update>

<Update label="2026-09-03" description="v1.7.2">
  ## BAS: Unnatural Mouse Detection
  - Unnatural mouse detection is now available and is recommended for every BAS integration. Track `MOUSE_MOVEMENT` and `MOUSE_CLICK` alongside keystrokes.
  - ReDem scores typing and mouse behaviour separately and uses the **lower** score for the data point. If both scores are equal, the typing category is used.
  - Mouse behaviour is flagged as `UNNATURAL_MOVEMENT` when it looks automated or scripted. Human-like pointer behaviour is `NATURAL_MOVEMENT`.
  - See the [Behavioral Analytics Score docs](/knowledge-base/quality-checks/behavior-analysis-score#mouse-movement-and-clicks) and the [Behavior Tracking guide](/api-reference/others/behavior-tracking) for details.

  ## BAS Typing
  - Typing detection is more accurate: more true positives for unnatural typing, and fewer false positives on genuine respondents. In particular, very slow typers are no longer flagged as unnatural when their typing is actually human.

  ## Respondent Details
  - When a quality-check score is missing for a respondent, the UI now explains why — for example a timeout, rate limit, content filter, or not enough data to score that check.
</Update>

<Update label="2026-08-27" description="v1.7.1">
  ## Quick Import
  - Quick Import is much faster for [supported survey platforms](/features/quick-import#supported-survey-tools), and quality-check suggestions are more accurate.

  ## Open-Ended Score
  - True-positive detection of AI-generated answers is higher. Answers that previously slipped through as valid are now flagged as `AI_SUSPECT` more reliably — for example: *"I like mangoes the most. It offers a rich and sweet flavor, along with a smooth and pleasant texture."*
  ...
  ## Duplicate Entrance Score
  - When you select the **Duplicate Entrance Score (DES)** as a quality check, ReDem now recommends which demographic questions to use.
  - Small UI polish in the import flow and respondent table so DES feels more consistent with the other quality checks.
  - See the [Duplicate Entrance Score guide](/knowledge-base/quality-checks/duplicate-entrance-score) and [Quick Import](/features/quick-import) for details.

  ## Coherence Score
  - Fixed a bug where respondent details sometimes highlighted the wrong questions or answers as incoherent. The view now shows only the questions that actually contribute to the incoherency.
</Update>

<Update label="2026-08-20" description="v1.7.0">
  ## Guest Access
  - You can now share a survey with people **outside your company**. From **Share**, type their email address to send an invite. Guests register or log in, then see only the surveys shared with them.
  - Guest access is **read-only**: guests can review results, but they cannot download data, change quality checks, clean, recalculate, or add respondents.
  - You can revoke guest access at any time. Emails that already belong to a ReDem user cannot be invited as guests.
  - Company admins can brand guest invites and the guest experience (logo, display name, and support email) under **Administration → Guest invite settings**.
  - Guest access is enabled per company. Contact ReDem if you would like it turned on.

  ## Open-Ended Score
  - ReDem now automatically turns off or restricts certain Open-Ended Score checks based on the **question type**, so expected answer patterns do not unfairly lower a respondent's score.
  - End-of-survey feedback questions run **only the AI-suspect check** (short or playful wrap-up answers are expected).
  - Short-label questions, such as unaided brand awareness, skip **duplicate detection** even when that check is enabled in the survey setup.
  - When an adjustment is applied, Respondent Details shows it under **Automatic scoring adjustments**.
  - See the [Open-Ended Score docs](/knowledge-base/quality-checks/open-ended-score#automatic-scoring-adjustments) for details.

  ## Small Fixes and Improvements
  - Sorting by Respondent ID now works for mixed IDs (for example `1`, `2`, `10`, `1a`, `10a`), not only numeric IDs.
  - You can recalculate at most **100** respondents at a time. Recalculate is disabled above that limit so the request cannot fail.
  - Question Text Row must be **1** or higher. Entering **0** now shows a validation error instead of failing later in the import.
  - Changing quality checks no longer fails when some respondents have no answer for a newly added check.
  - Empty typing or copy-paste events no longer cause the Behavioral Analytics Score to fail for that respondent.
  - DES respondent details now show IP in the comparison table (Match / Different) and use readable labels for demographics such as Age and Gender.
</Update>

<Update label="2026-08-13" description="v1.6.9">
  ## Quick Import
  - Quality-check suggestions in the Quick Import are now more accurate, and the analysis step finishes faster.

  ## Duplicate Entrance Score
  - You can now configure the **Duplicate Entrance Score (DES)** in the Quick Import. Map demographic columns — and an IP column, if you have one — in the same way as other quality checks.
  - The AI assistant does not currently recommend DES. If you would like to include it, enable it when you configure quality checks during import.
  - See the [Duplicate Entrance Score guide](/knowledge-base/quality-checks/duplicate-entrance-score) and [Quick Import](/features/quick-import) for details.

  ## BAS
  - The Behavioral Analytics Score now detects additional unnatural keystroke patterns.
</Update>

<Update label="2026-08-07" description="v1.6.8">
  ## Duplicate Entrance Score
  - The **Duplicate Entrance Score (DES)** is coming soon. It detects likely duplicate survey entrants using demographics and IP.
  - **API only at first** — send a DES datapoint on `POST /v3/addRespondent`. Quick Import support will follow.
...
<Update label="2026-07-31" description="v1.6.7">
...
<Update label="2026-07-23" description="v1.6.6">
  ## Quick Import
...
<Update label="2026-07-16" description="v1.6.5">
  ## Small Fixes
...
<Update label="2026-07-09" description="v1.6.4">
...
<Update label="2026-07-06" description="v1.6.3">
  ## BAS
  ...
  - See the [Quick Import guide](/features/quick-import#change-quality-check-settings) for details.
...
<Update label="2026-06-25" description="v1.6.2">
  ## Quick Import
...
<Update label="2026-06-18" description="v1.6.1">
  ## Teams
...
<Update label="2026-06-11" description="v1.6.0">
  ## Change Quality Check Settings
...
<Update label="2026-05-29" description="v1.5.9">
  ## Answer Distribution Analysis
...
<Update label="2026-05-22" description="v1.5.8">
...
<Update label="2026-05-11" description="v1.5.7">
  ## Respondent Table
...
<Update label="2026-05-04" description="v1.5.6">
  ## Company Settings
...
<Update label="2026-04-22" description="v1.5.5">
  ## Quick Import
  ...
  <Warning>It's still required to test the integration of your Survey with ReDem before going Live. The integration Agent has no guarantee that the integration will work out of the box.</Warning>
...
<Update label="2026-04-10" description="v1.5.4">
  ## API Keys
  ...
  <Warning>For API integrations in Decipher surveys you must use a Public API key. Otherwise you expose yourself to the risk of unauthorized access to your survey data.</Warning>
...
<Update label="2026-04-03" description="v1.5.3">
  ## Respondents
...
<Update label="2026-03-27" description="v1.5.2">
  ## Respondents
...
<Update label="2026-03-20" description="v1.5.1">
  ## Respondents
  ...
  ## Other improvements
  - **CHS & OES**: Requests Blocked by the AIs content filter are classified consistently as BAD_LANGUAGE; CHS scoring is adjusted so common, realistic answer patterns are not penalized as inconsistent.
  - **Import Assistant**: Hard limits on row and column counts clear errors when a file exceeds them.
</Update>

<Update label="2026-03-13" description="v1.5.0">
  ## Surveys
...
<Update label="2026-03-05" description="v1.4.9">
...
<Update label="2025-12-04" description="v1.3.3">
  ## Quick Import: OES Keywords Paste Support

  - **Flexible keyword input**: OES keywords can now be pasted directly into the UI during Quick Import with semicolons, commas, or newlines as separators.
...
<Update label="2025-11-06" description="v1.2.5">
  ## Quick Import: Multi-Sheet Excel Support
...
<Update label="2025-10-07" description="v1.2.2">
  ## Bulk Cleaning and Quick Import UX

  - Bulk cleaning: New backend endpoint and UI to clean all respondents from the survey results page with reusable settings.
  - Quick Import: Immediate progress bar after confirmation showing the total respondents being processed.
</Update>
...
<Update label="2025-09-19" description="v1.2.0">
  ## Quick Import: CSV and Excel with AI Suggestions
  ...
  👉 [Read the full guide](/features/quick-import)
