...
For each CHS data point, all question–answer pairs must be included as separate objects within the <code>interviewData</code> array. Each object should contain a <code>questionId</code> (as a string), a <code>question</code> (as a string) and an <code>answer</code> (as a string or number) to ensure compatibility with the <code>addRespondent</code> endpoint.
