...
ReDem supports two API key types based on where your integration runs:

- **Private (Server-Side)**: Use this key in your backend or secure server. It has full access to all APIs and must never be exposed in frontend code.
- **Public (Browser)**: Use this key in browser-based or frontend integrations. It is safe to expose but has limited access. For the following endpoints, you can use a Public key:
    - `POST /addRespondent`
    - `POST /getRespondent`
...
<Steps titleSize="h3">
  ...
  <Step title="Create an API Key">
    ...
    Give your API key a clear, descriptive name so you can easily identify its purpose later, and choose the appropriate API key type for your use case.
