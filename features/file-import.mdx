...
- **Labels & Sheets:** Single-sheet files or multi-sheet Excel files are supported. If your file has multiple sheets, you'll select which sheet contains the data and which contains the labels in Step 1. When using a single sheet, all data on that sheet must be fully labelled. All answers must be provided as plain text (not coded values).
