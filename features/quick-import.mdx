...
<Steps titleSize="h3">
  ...
  <Step title="Select Quality Checks and Questions">
    ...
    - **Choose Data Points:** Select the relevant open-ended question. Column pickers show both the **Header ID** and the **Question Text**; you can search either.
    ...
    - **Configure Other Quality Checks:**
      ...
      - **Coherence Score:** Select all questions you want evaluated for coherence. In **Survey Description & Exclusion Rules**, add survey context and any custom exclusion rules. Question text that appears 3 or more times is left out of the check. See the [Coherence Score guide](/knowledge-base/quality-checks/coherence-score).
      - **Time Score:** Select the variables that capture response time and/or interview length (in seconds or milliseconds).
      - **Duplicate Entrance Score:** Map an IP column, demographic columns (at least 4, up to 10), or both. IP-only is enough for the IP duplicate check. See the [Duplicate Entrance Score guide](/knowledge-base/quality-checks/duplicate-entrance-score) and [best-practice set-up](/knowledge-base/best-practices/best-practice-setup#duplicate-entrance-score).
  </Step>
  <Step title="Determine Cleaning Settings">
    ...
    **Recommended Settings**
    ...
    - Time Score is below 30 OR
    - Duplicate Entrance Score is below 40.
  </Step>
...
<Steps titleSize="h3">
  ...
  <Step title="Review AI-Recommended Quality Checks">
    ...
    **Duplicate Entrance Score (DES)**
    - Add the check when you review quality checks if it is not already selected. When DES is selected, ReDem recommends which demographic questions to map.
    - Review the suggested demographics, adjust as needed (at least 4, up to 10), and map an IP column if one is available. IP-only is enough for the IP duplicate check. See the [Duplicate Entrance Score guide](/knowledge-base/quality-checks/duplicate-entrance-score).
  </Step>
  <Step title="Determine Cleaning Settings">
    Decide whether to enable ReDem’s Automatic Data Cleaning. This feature applies exclusion recommendations based on predefined thresholds.
    - **If Enabled:** You can further customize the cleaning settings to suit your project.
...
Export your survey from IntelliSurvey and upload the Excel file directly to the Quick Import. ReDem detects the IntelliSurvey format and prepares the file for the import steps that follow.
