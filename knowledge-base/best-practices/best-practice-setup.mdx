...
The Coherence Score model's training cutoff is **May 18, 2026**. Avoid questions that rely on information after that date, such as: "Which smartphone model do you currently use?" Recently released products may not yet exist in the model's knowledge and can therefore lead to unjustified Coherence Score deductions.
...
To ensure the Duplicate Entrance Score functions reliably, please follow the recommendations below. In the Quick Import, map an IP column and/or these demographics when you configure the check. You can run DES as IP-only. The AI assistant does not currently recommend DES.
...
Prefer zip or city when available — fine-grained location is one of the strongest discriminators in the comparison window. For a demographic score, provide at least four demographics. If zip or city information is not available and only country-level location is used, include at least five demographics. Depending on which demographics you choose, six may be preferable so the combination remains distinctive enough to distinguish real duplicates from chance matches. IP-only DES does not need demographics.

#### **3. Do not use screening or single-group quota demographics**

Do not include demographics where only one answer is allowed to complete the survey. Those fields are the same for every completer, so they do not discriminate between entrants and can inflate duplicate matches.

This applies to screeners and to quotas that accept only one group. For example, if you ask about ethnicity and only let Hispanics proceed, ethnicity **must not** be used for the DES.

Balanced quotas are fine. If a quota only sets how many of each group you need, and answers still vary among completers, keep that field. A 50% male / 50% female quota does **not** mean gender should be excluded from the DES.
