...
For **Open-Ended Score (OES)** and **Grid-Question Score (GQS)** cleaning settings, `minDataPoints` is optional:
...
If you include an `OES` or `GQS` object in `cleaningSettings`, `activate` and `score` are required.
