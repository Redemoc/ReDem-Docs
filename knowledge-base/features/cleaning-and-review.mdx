...
For **Open-Ended Score (OES)**, **Grid-Question Score (GQS)**, and **Behavioral Analytics Score (BAS)** cleaning settings, `minDataPoints` is optional:

- **When `minDataPoints` is set:** A respondent is excluded by score only if their score falls below the threshold **and** they have at least that many valid data points.
- **When `minDataPoints` is omitted:** A respondent is excluded whenever their score falls below the threshold, regardless of how many valid data points they have.

If you include a quality-check object (`OES`, `CHS`, `GQS`, `TS`, or `BAS`) in `cleaningSettings`, `activate` and `score` are required.

<Note> **Example:** <br/> If you set the Open-Ended Score threshold to 40 with `minDataPoints` set to 2, then any interview with at least two open-ended responses and an overall Open-Ended Score below 40 is excluded. If `minDataPoints` is omitted, any interview with an Open-Ended Score below 40 is excluded, even with only one open-ended response. </Note>
...
- **Duplicate Entrance Score (DES)**: Respondents with a DES below 40 are excluded. Category-based DES exclusions (`DUPLICATE_IP`, `DUPLICATE_ENTRANT`) are off by default.
...
- **Duplicate Entrance Score (DES):** Threshold + optional category-based exclusion logic
  - **DES Categories:**
    - **Duplicate IP:**
    - **Duplicate Entrant:**
...
- **Duplicate Entrance Score Threshold**: Respondent’s DES is below the default (40) or a custom threshold.
