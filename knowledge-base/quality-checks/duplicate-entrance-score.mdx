...
DES is available in the **Quick Import**, the **Decipher Integration Agent**, and on **API v3**. It is not supported on v1/v2 `addRespondent` requests.
...
In the Quick Import, add the **Duplicate Entrance Score** quality check and map survey columns to demographic types (at least **4**, up to **10**). You can also map an IP column if one is available. Column names are searchable.

The AI assistant does not currently recommend DES. If you would like to include it, enable it when you configure quality checks. Your mapping is kept when you change quality checks, add respondents, or reuse a previous survey’s setup. See [File Import](/features/file-import).

### Decipher Integration Agent

DES is not suggested automatically. After you upload a Decipher XML file, click **Add Duplicate Entrance Score (DES)** and map demographic types to survey questions (at least **4**, up to **10**).

Entrance time is stored in the respondent’s browser when the survey starts and is sent with the integration request. IP is not collected in this flow. See [Decipher (Assistant)](/integration-guides/decipher).
