...
DES is available in **Quick Import** and on **API v3**. It is not supported on v1/v2 `addRespondent` requests.
...
During file import, add the **Duplicate Entrance Score** quality check and map survey columns to demographic types (at least **4**, up to **10**). You can also map an IP column if one is available. Column names are searchable.
