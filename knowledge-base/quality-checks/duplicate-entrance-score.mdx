...
- Otherwise, demographics are compared per peer. If either respondent has a demographic type the other does not, that pair scores **100**, even when the shared fields match. Exact matches keep a low (duplicate-like) score; similar age / year of birth can add a limited penalty; any hard mismatch treats that peer as different. The final score is the **minimum** over peers.
