...
During file import, add the **Duplicate Entrance Score** quality check and map survey columns to demographic types (at least **4**, up to **10**). Optionally map an IP column. Column names are searchable.

The AI assistant does **not** suggest DES automatically — add it yourself. Settings are kept when you change quality checks, add respondents, or reuse a previous survey’s setup. See [File Import](/features/file-import).
