...
<Note> **Important:** Scores are only calculated when there are at least 30 respondents, ensuring the median is statistically meaningful. If a datapoint has no recorded duration, or the Time Score service is unavailable, that datapoint is also left without a score.</Note>
